...
import type { Nutrients } from "@domain/products/types";
...
const getNutrientValue = (nutrients: Nutrients, key: (typeof nutrientKeys)[number]) => {
  switch (key) {
    case "protein":
...
const calculateEntryNutrients = (entry: MealEntry) => {
  ...
  nutrientKeys.forEach((key) => {
    setNutrientValue(nutrients, key, getNutrientValue(entry.product.nutrients, key) * factor);
  });
...
const addNutrients = (target: Nutrients, source: Nutrients) => {
  nutrientKeys.forEach((key) => {
    setNutrientValue(target, key, getNutrientValue(target, key) + getNutrientValue(source, key));
  });
};
