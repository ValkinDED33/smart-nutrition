...
- с облачным AI: backend включает `/api/ai`, хранит историю диалога в настроенном backend storage и использует vision-capable провайдеры для фото еды

Важно: фото еды не должно быть шаблоном. `/api/photo-analysis` сначала пробует backend vision-provider с языком профиля, типом приёма пищи и ограничениями пользователя. Если фото плохое или провайдер не уверен, приложение честно просит сделать более светлое/чёткое фото или вручную поправить ингредиенты перед сохранением.
