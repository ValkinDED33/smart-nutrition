...
- Replaced the stale Resend-only email diagnostic/docs with provider-neutral transactional email diagnostics: `npm run email:check` now verifies the canonical Brevo-primary/Resend-reserve email service without printing secrets.
...
- Expanded Admin Center user management into an operational account-state view: admins now see cloud-backed user totals, online sessions, email confirmation health, attention counts, protected roles, created dates, last sessions, security state, and ban details instead of status-only user cards.
